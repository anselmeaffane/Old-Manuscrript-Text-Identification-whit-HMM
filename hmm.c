         ...
         int maxChars)
{
    ...
    while(fgets(temp, MAX_CHARS, in) != NULL)
    {
        ...
        for(i = thisStartPos; i <= len; ++i)
        {
            ...
            for(j = 0; j < M; ++j)
            {
            	//added by Affane 07/12/2021
            	//int result = strncasecmp(&temp[i], &alphabet[j], 1);//strncasecmp() — Compare Strings without Case Sensitivity
                //int result = strncmp(&temp[i], &alphabet[j], 1);
                //if(strncasecmp(&temp[i], &alphabet[j], 1) == 0)//ORIGINAL CONSTRAINT
                if(strncmp(&temp[i], &alphabet[j], 1) == 0)//added by Affane 10/12/2021
                //if((strcmp(&temp[i],&alphabet[j]))==0)
                //if(result != 0)
                //if(strncmp(&temp[i], &alphabet[j], 1) == 0)
                /*std::string str1, str2;
                string str1(&temp[i]);
		 string str2(&alphabet[j]);
                if(str1.compare(str2) != 0)*/
                //if(strncat(&temp[i], &alphabet[j], 1) != 0)//strncat() — Concatenate Strings//added by Affane 10/12/2021
                {
                    ++totalNum;
                    ...
                    int maxChars)
{
    ...
    while(fgets(temp, MAX_CHARS, in) != NULL)
    {
        ...
        // ignore leading spaces
        while((strncmp(&temp[thisStartPos], space, 1) == 0) && (thisStartPos < len))//added
	//while((strncasecmp(&temp[thisStartPos], space, 1) == 0) && (thisStartPos < len))
        {
            ++thisStartPos;
        }
        ...
        for(i = thisStartPos; i <= len; ++i)
        {
            ...
            for(j = 0; j < M; ++j)
            {
                //if(strncasecmp(&temp[i], &alphabet[j], 1) == 0)//Origine
                if(strncmp(&temp[i], &alphabet[j], 1) == 0)//Update 10/12/2021 by Affane et al.
                {
                    ++totalNum;
                    if(totalNum >= startChar)
                    {
